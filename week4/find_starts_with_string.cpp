...
pair<RandomIt, RandomIt> FindStartsWith(
    ...
    const string& prefix) {
	char next_char = char(prefix.back() + 1);
	string next_str = prefix.substr(0, prefix.size() - 1) + string(1, next_char);

	auto left = lower_bound(range_begin, range_end, prefix);
	auto right = lower_bound(range_begin, range_end, next_str);
	return {left, right};
}
